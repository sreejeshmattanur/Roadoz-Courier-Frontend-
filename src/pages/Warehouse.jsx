...
import {
  ...
  Settings,
  Trash2,
  Lock, // Added Lock icon for password
} from "lucide-react";
...
export function Warehouse() {
  ...
  const handleEdit = (warehouse) => {
    ...
    setFormData({
      ...
      password: "", // Usually keep password empty during edit for security/backend logic
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (Array.isArray(err)) {
        ...
        // Handle specialized error format from backend
        errorMessage = err.detail.map(d => `${d.loc[1]}: ${d.msg}`).join(", ");
      }
      ...
      {loading ? (
        ...
        <>
                  ...
                  {new Date(w.created_at).toLocaleDateString()}
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <p className="font-bold text-text-main text-sm">{w.nickname}</p>
                        ...
                        <td className="px-6 py-4 text-sm text-text-main">{w.contact_name}</td>
          ...
          <div className="md:hidden space-y-4">
            {filteredItems.map((w) => (
              <Card key={w.id} className="bg-card-bg border-border-subtle overflow-hidden">
                <CardContent className="p-4 space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-text-main">{w.nickname}</p>
                      <p className="text-[11px] text-text-muted">{w.contact_name}</p>
                    </div>
                  </div>
                  <div className="space-y-2 text-[12px]">
                    ...
                    <div className="flex items-start gap-2 text-text-muted">
                      ...
                      <div>
                        <div>{w.city}, {w.state}</div>
                        <div>{w.country} - {w.pincode}</div>
                      </div>
                    </div>
                    ...
                    <div className="pt-3 border-t border-border-subtle flex gap-2">
                      <Button
                        onClick={() => handleEdit(w)}
                        variant="ghost"
                        className="flex-1 h-10 rounded-xl border border-yellow-500/30 bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-400 text-xs font-bold"
                      >
                      ...
                      <Button
                        onClick={() => handleDelete(w.id)}
                        variant="ghost"
                        className="flex-1 h-10 rounded-xl border border-red-500/30 bg-red-500/10 hover:bg-red-500/20 text-red-400 text-xs font-bold"
                      >
