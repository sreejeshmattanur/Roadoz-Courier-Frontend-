...
import {
    ...
    captureLocationApi // Ensure this is exported in your apiCalls.js
...
// --- 1. INITIALIZATION: GPS & LOCATION CAPTURE API ---
    useEffect(() => {
        if ("geolocation" in navigator) {
            const watchId = navigator.geolocation.watchPosition(
                async (pos) => {
                    // Extract values directly from the coordinates object
                    ...
                    // Trigger capture API only if we have valid numbers and haven't captured yet
                    if (!initialCaptureDone.current && rawLat && rawLng) {
                        initialCaptureDone.current = true;
                        
                        try {
                            // Ensure we are sending pure numbers, not strings or nulls
                            const payload = { 
                                latitude: Number(rawLat), 
                                longitude: Number(rawLng) 
                            };

                            console.log("Sending Location Payload:", payload); // Debugging

                            await captureLocationApi(payload);
                            setIsLocationCaptured(true);
                        } catch (err) {
                            console.error("Location capture failed", err);
                            // If it fails, allow a retry on next position update
                            initialCaptureDone.current = false; 
                            toast.error("Location verification failed. Retrying...");
                        } finally {
                ...
                (err) => {
                    ...
                    toast.error("GPS access is required to use this dashboard.");
                    setIsInitializing(false);
                },
                { 
    ...
    // --- 2. DATA API CALLS (Triggered only after isLocationCaptured is true) ---
    ...
    // --- LOADING SCREEN WHILE INITIALIZING GPS/LOCATION CAPTURE ---
    if (isInitializing) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-[#09090b] text-white">
                <Loader2 className="animate-spin text-primary mb-4" size={48} />
                <p className="text-sm font-bold uppercase tracking-widest animate-pulse">Initializing Security Context...</p>
                <p className="text-[10px] text-gray-500 mt-2">Capturing GPS coordinates and verifying location...</p>
            </div>
        );
    }
