...
import {
  ...
  Globe, Truck, Zap, Ban, FileText, Scale, Home, Building2, File, 
  Ban as BanIcon
} from "lucide-react";
...
export default function NewOrder() {
  ...
  // Insurance Calculation: 1.8% of Total if >= 1000
  useEffect(() => {
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-end">
            ...
            <div className="flex gap-4 flex-1">
              {paymentMethod !== "Prepaid" && <div className="flex-1"><label className="text-[10px] font-bold text-text-muted uppercase ml-1">Amt</label><input type="number" value={paymentMethod === "COD" ? codAmount : paymentMethod === "To Pay" ? toPayAmount : creditAmount} onChange={(e) => paymentMethod === "COD" ? setCodAmount(e.target.value) : paymentMethod === "To Pay" ? setToPayAmount(e.target.value) : setCreditAmount(e.target.value)} className={inputClass} /></div>}
            ...
            <div key={pkg.id} className="grid grid-cols-2 md:grid-cols-7 gap-4 items-end border-b border-border-subtle pb-6 last:border-0">
              ...
              <div className="space-y-1">
                ...
                <div className="flex">
                  ...
                  <select value={pkg.weight_unit} onChange={(e) => handlePackageChange(pkg.id, "weight_unit", e.target.value)} className="bg-dashboard-bg border border-l-0 border-border-subtle rounded-r-lg px-1 text-[10px] text-primary outline-none"><option value="kg">kg</option><option value="g">g</option></select>
                </div>
              </div>
        ...
        {isOtherDetailsOpen && (
          <CardContent className="p-6 pt-0 border-t border-border-subtle/20 animate-in slide-in-from-top-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
              ...
              <div className="space-y-1"><label className="text-[10px] font-bold text-text-muted uppercase ml-1">Bill Amount</label><input type="number" value={otherDetails.amount} onChange={(e) => setOtherDetails({...otherDetails, amount: e.target.value})} className={inputClass} /></div>
              <div className="md:col-span-2 flex items-center justify-between bg-dashboard-bg/20 p-4 rounded-xl border border-border-subtle">
                ...
                <button onClick={() => setIsGstExempt(!isGstExempt)} className={cn("w-12 h-6 rounded-full transition-all relative", isGstExempt ? "bg-primary" : "bg-white/5 border border-border-subtle")}><div className={cn("w-4 h-4 rounded-full bg-white absolute top-0.5 transition-all shadow-md", isGstExempt ? "left-7" : "left-0.5")} /></button>
              </div>
            </div>
          </CardContent>
        )}
